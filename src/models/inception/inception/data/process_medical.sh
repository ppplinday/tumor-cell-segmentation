...
if [ -z "$1" ]; then
  echo "Usage: process_medical.sh [data dir]"
  exit
fi

# Create the output and temporary directories.
DATA_DIR="${1%/}"
ANNOTATION_DIRECTORY="${DATA_DIR}/15_15_annotation"
...
# Expands the data into the images6/ directory and rename it as the
# train directory.
IMAGES="${DATA_DIR}/images/"
rm -rf "${TRAIN_DIRECTORY}" "${VALIDATION_DIRECTORY}"
mv "${IMAGES}" "${TRAIN_DIRECTORY}"

# Generate a list of 2 labels: 0, 1
...
while read LABEL; do
  ...
  if [[ $LABEL == 0 ]]; then
    VALIDATION_IMAGES=$(ls -1 "${TRAIN_DIR_FOR_LABEL}" | shuf | head -15000)
  elif [[ $LABEL == 1 ]]; then
    VALIDATION_IMAGES=$(ls -1 "${TRAIN_DIR_FOR_LABEL}" | shuf | head -15000)
  fi
